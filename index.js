...
if (!fs.existsSync(CREDS) && config.SESSION_ID && config.SESSION_ID.includes("ABDULLAH-BOTZ~")) {
  try {
    ...
    console.log("♻️ Session restored from SESSION_ID");
  } catch(e) {
    console.log("⚠️ SESSION_ID invalid, will use pairing code instead");
  }
}
  ...
  sock.ev.on('connection.update', (update) => {
    ...
    if (connection === 'close') {
      ...
      if (statusCode === DisconnectReason.loggedOut) {
        ...
        console.log("⚠️ Bad MAC error. Deleting session and reconnecting...");
        fs.rmSync(__dirname + '/auth_info_baileys', { recursive: true, force: true });
        connectToWA();
      } else {
