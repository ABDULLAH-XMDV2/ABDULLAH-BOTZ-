...
app.get('/pair', async (req, res) => {
  try {
    ...
    if (!number || number.length < 10) {
      return res.status(400).json({ error: 'Valid phone number required (with country code, example: 923041956023)' });
    }
    ...
    // ✅ REAL FIX: Use globalSock directly — it must be ready
    // Wait up to 15s for globalSock to be available
    let waited = 0;
    while (!globalSock && waited < 15000) {
      await new Promise(r => setTimeout(r, 500));
      waited += 500;
    }

    if (!globalSock) {
      return res.status(503).json({ error: 'Bot socket not ready yet, please wait 10 seconds and try again' });
    }

    // ✅ requestPairingCode — this is the REAL Baileys function
    console.log(`[PAIR] Requesting pairing code for +${number}`);
    const raw = await globalSock.requestPairingCode(number);
    
    if (!raw) {
      return res.status(500).json({ error: 'Failed to generate pairing code. Make sure bot is not already linked.' });
    }

    // Format: ABCD-EFGH
    const code = typeof raw === 'string' && raw.length === 8
    ...
    if (e.message?.includes('already')) {
      return res.status(400).json({ error: 'Bot is already linked to a device. Delete auth_info_baileys folder first.' });
    }
    return res.status(500).json({ error: e.message || 'Server error, try again' });
  }
});
