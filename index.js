...
function loadLocalFiles() {
  ...
  if (fs.existsSync(path.join(__dirname, 'lib', 'arslan.html'))) {
    console.log("✅ arslan.html found in lib folder");
  } else {
    console.log("[WARN] arslan.html not found in lib folder - path: " + path.join(__dirname, 'lib', 'arslan.html'));
  }
...
// ✅ FIX: Serve entire lib folder as static (so arslan.html and its assets load)
app.use('/lib', express.static(path.join(__dirname, 'lib')));

// ✅ FIX: Serve arslan.html on root '/' using absolute __dirname path
app.get('/', (req, res) => {
    ...
    res.send(`
      ...
      <body>
        <div class="container">
          ...
          <p style="color:red">[WARN] arslan.html not found at: ${path.join(__dirname, 'lib', 'arslan.html')}</p>
        </div>
      </body>
      </html>
    `);
...
// ✅ FIX: /lib/arslan.html directly serves the file (no redirect needed)
...
app.listen(port, '0.0.0.0', () => {
  ...
  console.log('[INFO] Serving from: ' + __dirname);
  console.log('[INFO] lib folder: ' + path.join(__dirname, 'lib'));
  console.log('[INFO] arslan.html exists: ' + fs.existsSync(path.join(__dirname, 'lib', 'arslan.html')));
});

// ── PAIRING API ROUTES ──────────────────────────────────────────────────────

// GET /pair?number=923001234567  → returns { code: "ABCD-WXYZ" }
app.get('/pair', async (req, res) => {
  try {
    ...
    if (!globalSock) {
      return res.status(503).json({ error: 'Bot is starting up, please wait a moment and try again' });
    }
    // Check if already connected
    const credsPath = path.join(__dirname, 'auth_info_baileys', 'creds.json');
    if (fs.existsSync(credsPath) && botConnected) {
      return res.status(200).json({ connected: true, message: 'Bot is already connected!' });
    }
    const raw = await globalSock.requestPairingCode(number);
    if (!raw) return res.status(500).json({ error: 'Failed to generate code, try again' });
    const code = raw.length === 8 ? raw.slice(0,4) + '-' + raw.slice(4) : raw;
    res.json({ code, expires: 60, number });
    console.log(`Web pair request: +${number} -> ${code}`);
  } catch (e) {
    console.error('Pair route error:', e.message);
...
async function connectToWA() {
  ...
  const sock = makeWASocket({
    ...
    // Show QR only as fallback if no phone number configured
    printQRInTerminal: needsPairing && !config.OWNER_NUMBER,
    browser: ['Ubuntu', 'Chrome', '20.0.04'],
    syncFullHistory: true,
    auth: authState,
    version: version
  });
  globalSock = sock; // expose to web API routes

  // ── AUTO PAIRING CODE ──────────────────────────────────────────────────────
  if (needsPairing && config.OWNER_NUMBER) {
    const phoneNumber = config.OWNER_NUMBER.replace(/[^0-9]/g, '');
    console.log(`\n╔══════════════════════════════════════╗`);
    console.log(`║   🔗 ABDULLAH-BOTZ PAIRING SYSTEM   ║`);
    console.log(`╠══════════════════════════════════════╣`);
    console.log(`║  📞 Number : +${phoneNumber.padEnd(22)}║`);
    console.log(`║  ⏳ Requesting pairing code...       ║`);
    console.log(`╚══════════════════════════════════════╝\n`);

    // Small delay so socket registers before requesting
    ...
    try {
      ...
      console.log(`║                                      ║`);
      console.log(`║  [WARN]  Code expires in 60 seconds!    ║`);
      console.log(`╚══════════════════════════════════════╝\n`);
    } catch (err) {
      console.log(`\n❌ Pairing code error: ${err.message}`);
      console.log(`   Make sure OWNER_NUMBER in config.env is correct`);
      console.log(`   Example: OWNER_NUMBER=923041956023\n`);
    }
  }
    ...
    try {
      ...
      if (msg.key && msg.key.remoteJid === 'status@broadcast') {
        ...
        if (config.AUTO_STATUS_REPLY === 'true' && msg.key.participant) {
          try {
            const statusReplyMsg = botConfig.STATUS_MSG || 'Thanks for status! <3';
