...
async (conn, mek, m, { from, contextInfo, reply }) => {
    try {
        ...
        let envSettings = `╭━━━〔 *ABDULLAH-BOTZ-ENV* 〕━━━┈⊷
┃▸╭───────────
┃▸┃๏ *𝔼ℕ𝕍 𝕊𝔼𝕋𝕋𝕀ℕ𝔾𝕊 📡*
┃▸└───────────···๏
╰────────────────┈⊷
...
//============================List===========================================
// ============ CMD COMMAND - shows all cmds line by line with category headers ============
cmd({
  pattern: "cmd",
  alias: ["cmds", "commands", "list"],
  react: "📋",
  desc: "Show all commands line by line with category",
  category: "main",
  use: ".cmd",
  filename: __filename
}, async (conn, mek, m, { from, pushname, reply, contextInfo }) => {
  try {
    ...
    for (const [cat, label] of Object.entries(categories)) {
      ...
      text += `*╰── Total: ${cmds.length} ──*\n\n`;
    }

    text += `> *ABDULLAH-BOTZ* | Type ${config.PREFIX}cmd to see this list`;
...
cmd({
  ...
  try {
    ...
    text += `> *ABDULLAH-BOTZ* | Type ${config.PREFIX}menu to see this list`;
...
async (conn, mek, m, { from }) => {
    try {
        ...
        const ownerNumber = '+923041956023'; // Replace this with the actual owner number
        const ownerName = 'ABDULLAH'; // Replace this with the owner's name
        const organization = 'ABDULLAH-BOTZ CODERS'; // Optional: replace with the owner's organization
        ...
        await conn.sendMessage(from, {
            ...
            contextInfo: {
                mentionedJid: [ownerNumber.replace('+923041956023', '') + '+923041956023@s.whatsapp.net'], // Mention the owner
...
try{
    ...
    let ping = await conn.sendMessage(from , { text: '*_ABDULLAH-BOTZ..._*'  }, { quoted: mek } )
...
try{
let dec = `> ABDULLAH-BOTZ REPO INFO 🪀

╭⦁⦂⦁*━┉━┉━┉━┉━┉━┉━⦁⦂⦁
┃ 𝙾𝚆𝙽𝙴𝚁 𝙽𝚄𝙼𝙱𝙴𝚁: ${bot.OWNER_NUMBER}
┃ 
┃ ABDULLAH-BOTZ REPO: ${bot.REPO_LINK} 
┃
...
async (conn, mek, m, { from, quoted, body, isCmd, command, args, q, isGroup, sender, senderNumber, botNumber2, botNumber, pushname, isMe, isOwner, groupMetadata, groupName, participants, groupAdmins, isBotAdmins, isAdmins, reply }) => {
    try {
        ...
        let autoreact = config.AUTO_REACT === 'true' ? '♻ 𝙾𝙽' : '🚫 𝙾𝙵𝙵';

        const vv = await conn.sendMessage(from, {
            image: { url:bot.ALIVE_IMG},
            caption: `> ABDULLAH-BOTZ Settings\n
┏━━━━━━━━━━━━━━━━━━┓
┃╭┈────────━━━━───╮
┣┣Work Mode : *${work}*
